...
if (isset($_POST['wfhuserperm_approval_email'])) {
  $colname_find_exstwfhuserfbuserprofile_email = mysqli_real_escape_string($idsconnection_mysqli, trim($_POST['wfhuserperm_approval_email']));
}
...
$post_risk_factor_lvl =  mysqli_real_escape_string($idsconnection_mysqli, trim($_POST['risk_factor_lvl']));
...
mysql_select_db($database_wfh_connection, $wfh_connection);
$query_find_exstwfhuser_approval = "SELECT * FROM `idsids_wefinancehere`.`wfhuser_approvals_perms` WHERE `wfhuser_approvals_perms`.`wfhuserperm_approval_email` = '$post_wfhuserperm_approval_email' AND `wfhuser_approvals_perms`.`wfhuserperm_vehicle_id` = '$post_vid'";
$find_exstwfhuser_approval = mysqli_query($idsconnection_mysqli, $query_find_exstwfhuser_approval, $wfh_connection);
$row_find_exstwfhuser_approval = mysqli_fetch_assoc($find_exstwfhuser_approval);
$totalRows_find_exstwfhuser_approval = mysqli_num_rows($find_exstwfhuser_approval);
...
`wfhuserperm_approval_risk_factor_lvl` = '$post_risk_factor_lvl',
`wfhuserperm_fbid` = '$post_wfhuserperm_fbid',
`wfhuserperm_ip` = '$ip',
`wfhuserperm_browser` = '$browser',
`wfhuserperm_ourrank` = '$wfhuserperm_ourrank',
`wfhuserperm_okay_perm` = '1',
`wfhuserperm_when_date` = '$server_time',
`wfhdelrperm_okay_perm` = '0',
...
echo $create_wfhuser_approval_permission_sql;

	$query_create_wfhuser_approval_permission_sql_saved = mysqli_query($wfh_connection_mysqli, $create_wfhuser_approval_permission_sql);
