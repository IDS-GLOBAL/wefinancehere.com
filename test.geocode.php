...
<option value="29.0" <?php if (29.0 == $row_find_existingsession_approval['wfhuser_approval_apr']){echo "selected=\"selected\"";} ?>>No Credit: (0 - ?) - I am not Sure</option>
    ...
    <script src="jSon/cluster-junk.json"></script>
    <script src="js/plugin/markercluster/markerclusterer.js" type="text/javascript"></script>
    <script src="https://unpkg.com/axios/dist/axios.min.js" type="text/javascript"></script>  <!-- https://github.com/axios/axios -->
    <script>
   ...
   function initMap() {	  
	  	...
	  	if('geolocation' in navigator)
	  	{
			...
			navigator.geolocation.getCurrentPosition(function(position) {
				console.log(position); // This gives you the position of everything available. intresting to see what pops up in a mobile browser. 
				...
				document.getElementById('latitude_results').textContent = mylat;
				
				document.getElementById('longitude_results').textContent = mylong;
				

				//  This is reversing geocoder of geolocation to prepare for map display.
				// https://maps.googleapis.com/maps/api/geocode/json?latlng=40.714224,-73.961452&key=AIzaSyCYwsd0NKRysnUVB7BOi-4lRd7cq1QwS6Y
				

				// Make a request for a user with a given ID
				axios.get('https://maps.googleapis.com/maps/api/geocode/json?latlng='+ mylat + ','+ mylong + '&key=AIzaSyCYwsd0NKRysnUVB7BOi-4lRd7cq1QwS6Y')
				  .then(function (response) {
					...
					console.log(response.data.results[0].address_components[4].short_name);
					...
					for(var i =0;i < addressComponents.length;i++){
						addressComponentsOutput += `<li class="list-group-item"><strong>${addressComponents[i].types[0]}</strong>: ${addressComponents[i].short_name}</li>`;
					}
    ...
    var options = {
		...
		// New Map
		var map = new google.maps.Map(document.getElementById('usmap'), options);
		...
		*/
		...
		var markers = [
			{
			coords:{lat:33.753746,lng:-84.386330},
			iconImage:'https://developers.google.com/maps/documentation/javascript/examples/full/images/beachflag.png',
			content:'<h2>Atlanta GA</h2>'
			},
			{
			coords:{lat:33.7939719,lng:-84.26465329999999},
			content:'<h2>Central Auto</h2>'
			},
			{
			coords:{lat:33.77451011,lng:-84.28028169999999},
			content:'<h2>Atlanta Auto Star Finance</h2>'
			}
    ...
    <script async defer
	src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCYwsd0NKRysnUVB7BOi-4lRd7cq1QwS6Y&callback=initMap">
    </script>
